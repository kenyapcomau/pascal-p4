          ...
          BEGIN VARIABLE(FSYS + [COMMA,RPARENT]); LOADADDRESS;                  PASCP 2231
            ...
            IF GATTR.TYPTR <> NIL THEN                                          PASCP 2233
              WITH GATTR.TYPTR' DO                                              PASCP 2234
                IF FORM = POINTEL THEN                                          PASCP 2233
              WITH GATTR.TYPTR' DO                                              PASCP 2234
                IF FORM = POINTER THEN                                          PASCP 2235
                  BEGIN                                                         PASCP 2236
                    IF ELTYPE <> NIL THEN                                       PASCP 2237
                      BEGIN LSIZE := ELTYPE'.SIZE;                              PASCP 2230
                  END                                                           PASCP 2241
...
FOR I:=33 TO 34 DO                                                              PASCP 3430
  ...
  PROCEDURE INITTABLES;                                                         PASCP 3541
    ...
    FOR I := ORDMINCHAR TO ORDMAXCHAR DO SOP[CHR(I)] := NOOP;                   NEWB     4
   ...
   MN[60] :=# UJC#;                                                             P      564
    END (*INSTRMNEMONICS*) ;                                                    PASCP 3624
     ...
     BEGIN                                                                      P      569
       ...
       CHARTP[#/#] := SPECIAL; CHARTP[#(#] := CHLPAREN;                         P      591
       CHARTP[#)#] := SPECIAL; CHARTP[#$#] := SPECIAL;                          P      592
       CHARTP[#=#] := SPECIAL; CHARTP[# #] := CHSPACE;                          P      593
       CHARTP[#,#] := SPECIAL; CHARTP[#.#] := CHPERIOD;                         P      594
       CHARTP[####] := CHSTRQUO; CHARTP[#[#] := SPECIAL;                        P      595
       CHARTP[#]#] := SPECIAL; CHARTP[#:#] := CHCOLON;                          P      596
       CHARTP[#'#] := SPECIAL; CHARTP[#;#] := SPECIAL;                          P      597
       CHARTP[#<#] := CHLT; CHARTP[#>#] := CHGT;                                P      598
